...
export interface ArchitectureBoundaryScannerOptions {
  repoRoot: string;
  rules?: ArchitectureBoundaryRuleSet;
  scanRoots?: readonly string[];
  workspacePackageRoots?: ReadonlyMap<string, string>;
}

const defaultScanRoots = ["apps", "packages"];
const sourceExtensions = [".ts", ".tsx", ".mts", ".cts"];
const ignoredDirectories = new Set([".git", ".next", "coverage", "dist", "node_modules"]);
const defaultWorkspacePackageRoots = new Map([
...
export function scanArchitectureBoundaries(
  options: ArchitectureBoundaryScannerOptions
): ArchitectureBoundaryViolation[] {
  const scanRoots = options.scanRoots ?? defaultScanRoots;
  const rules = options.rules ?? architectureBoundaryRules;
  const workspacePackageRoots =
    options.workspacePackageRoots ?? defaultWorkspacePackageRoots;
  const files = scanRoots.flatMap((root) => walk(join(options.repoRoot, root)));
  const violations: ArchitectureBoundaryViolation[] = [];

  for (const absoluteFilePath of files) {
    const source = readFileSync(absoluteFilePath, "utf8");
    const importingFile = toRepoPath(options.repoRoot, absoluteFilePath);
    const sourceCategory = classifyPath(importingFile, rules);
    ...
    for (const reference of getImportReferences(source)) {
      ...
      const resolvedTargetPath = toRepoPath(options.repoRoot, resolvedTarget);
      const targetCategory = classifyPath(resolvedTargetPath, rules);
...
export function getDefaultArchitectureRepoRoot() {
...
function classifyPath(
  path: string,
  rules: ArchitectureBoundaryRuleSet
): ArchitectureBoundaryCategory | undefined {
