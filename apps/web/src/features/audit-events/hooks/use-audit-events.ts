...
import type { EventListWorkspaceQuery } from "@/src/features/audit-events/domain/query";
...
export function useAuditEvents(
  ...
  workspace: Required<EventListWorkspaceQuery>
) {
  const service = useMemo(
    () =>
      createAuditEventsService(
        createAuditEventsClient(createBrowserApiClient(), workspace)
      ),
    [workspace]
  );
