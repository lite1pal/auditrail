...
function renderApiRoutes(context: ReturnType<typeof createTemplateContext>) {
  return [
    ...
    "  app: FastifyInstance,",
    `  options: { service: ReturnType<typeof create${context.pascalName}Service> }`,
    ") {",
    `  app.get("${context.apiBasePath.replace("/api", "")}", async (request, reply) => {`,
    ...
    "  });",
    "",
    `  app.post("${context.apiBasePath.replace("/api", "")}", async (request, reply) => {`,
    "    const params = organizationParamsSchema.safeParse(request.params);",
    "",
    "    if (!params.success) {",
    '      return reply.code(400).send({ error: "invalid_request" });',
    ...
    "  });",
    "",
    `  app.get("${context.apiBasePath.replace("/api", "")}/:id", async (request, reply) => {`,
    ...
    "  });",
    "",
    `  app.patch("${context.apiBasePath.replace("/api", "")}/:id", async (request, reply) => {`,
    "    const params = resourceIdParamsSchema.safeParse(request.params);",
    "",
    "    if (!params.success) {",
    '      return reply.code(400).send({ error: "invalid_request" });',
...
function renderApiRoutesTest(context: ReturnType<typeof createTemplateContext>) {
  return [
    ...
    "    const app = Fastify();",
    "",
    `    await register${context.pascalName}Routes(app, {`,
    "      service: {",
    "        async create() {",
    ...
    `      url: "${context.apiBasePath.replace("/api", "")}"`,
    "    });",
    "",
...
function renderWebApiClient(context: ReturnType<typeof createTemplateContext>) {
  return [
    ...
    "          body,",
    '          method: "POST",',
    `          path: "${context.apiBasePath}/${'${organizationId}'}".replace("//", "/") as never`,
    "        })",
    "      );",
    "    },",
    ...
    `      return ${context.resource.resource}RecordSchema.parse(`,
    "        await apiClient.request({",
    `          path: "${context.apiBasePath}/${'${organizationId}'}/${'${id}'}".replace("//", "/") as never`,
    "        })",
    "      );",
    "    },",
